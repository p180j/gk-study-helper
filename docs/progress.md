...
- [x] 首页信息层级收口：移除重复的“为什么这样安排”和“能力画像建立度”，将全局最高优先级学习问题迁移为“今天重点解决”；能力页移除 AI 教练入口和全局问题卡，仅保留能力画像、模块详情与针对训练
- [x] 本次首页/能力页 JS 语法、能力视图测试通过；微信开发者工具 CLI 实际重新载入项目成功
...
- [x] GitHub `fei98/civil-service-exam-prep` 5000 条源题全部完成转换和导入尝试；主库实际新增 4973 道（ACTIVE 4127 / DRAFT 846），另 27 道被内容哈希去重
- [x] 图片依赖题统一进入 DRAFT，保留原题图片地址；未将缺图题投放到日常训练，避免污染 AnswerRecord 与能力计算
- [x] 新增可复用的 JSON 转 CSV、江西“试卷 PDF + 解析 PDF / OCR 文本”配对转换脚本；脚本语法检查通过
- [x] 江西 2020 三套卷共处理 385 题，题号与答案全部对应；跨卷去重后实际新增 206 道（ACTIVE 107 / DRAFT 99）
- [x] 江西 2021 卷通过本地 OCR 提取并逐题校验：135 道题目、答案、解析全部配对，无缺号；实际新增 135 道（ACTIVE 124 / DRAFT 11）
- [x] 江西 2022 卷 135 题题干、选项与公开答案表完整对应并导入 DRAFT；因缺少可校验的逐题解析，未投放为可训练题
- [x] 从上游 `mpbfx/gongkao` 完整处理江西 2023 / 2024 / 2025 各 135 题、2026 130 题，共 535 题；实际新增 242 道，其余 293 道由内容哈希确认已存在
- [x] 江西 2020—2026 共 1190 条省考源题全部完成导入；重复执行导入时全部返回重复、题库不增长，幂等校验通过
- [x] 主库题目总数实测 5785 道，普通可训练库存 4629 道；五个行测模块均有真实库存
- [x] Java 11 `mvn clean test` 实际通过：280 项，0 失败

## 管理控台产品化重构（本轮）

- [x] Spring Boot 统一托管入口 `/admin/`、`/admin/index.html`、脚本与样式资源，后台无需单独启动前端服务
- [x] 左侧导航重构为工作台、内容中心、学习运营、考试运营、系统五组，共 16 个真实业务视图
- [x] 新增运营工作台：真实展示可训练题、待审核内容、能力建立度、学习问题、今日计划、待处理事项与系统健康
- [x] 题库页面合并行测与申论入口，以模块库存、未使用量、题目质量和详情为主要信息，不再作为单纯 CRUD 表格
- [x] 内容采集形成采集概览、来源、文件上传、异常处理、运行日志完整工作流；真实触发来源 #7，日志 #8 执行成功
- [x] 知识体系可查看模块与子知识点库存、低库存风险，并可跳转关联题目
- [x] 今日计划展示真实目标题量、已完成题量、任务目的、问题来源与安排原因
- [x] 学习记录统一行测、申论、政治阅读、模考入口；单题详情可追踪 AnswerRecord → ErrorDiagnosis → Ability → LearningProblem → DailyPlan 影响
- [x] Ability / LearningProblem / ErrorDiagnosis 分别提供运营诊断页面；新增只读 `GET /api/error-diagnoses`，不复制业务 Service、不修改诊断算法
- [x] 模考试卷、模考记录、模考分析形成连续入口；无模考记录时明确显示真实空状态
- [x] AI 配置仅展示脱敏密钥与真实测试状态；系统状态聚合关键接口、采集失败和最近真实异常
- [x] 新增统一中文业务展示映射，主要页面不再把内部英文枚举作为用户主信息
- [x] Edge 无头真实浏览器逐页访问全部管理视图并保存 16 张验收截图至 `docs/acceptance-screenshots/admin-product-*.png`
- [x] JDK 11 `mvn clean test` 实际通过：281 项，0 失败；`admin-web/app.js` 与 `display.js` 语法检查通过
- [x] 最终 Spring Boot 使用现有 MySQL 正常启动，数据库连接、MyBatis、静态资源和新增错因查询接口均正常
